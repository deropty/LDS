...
def print_csv_format(results, task=None):
    """
    ...
    if task is None:
        task = "Dataset"
    metrics = [task] + [k for k in results[list(results.keys())[0]]]
...
def ldsprint_csv_format(results):
    """
    Print main metrics in a format similar to Detectron,
    so that they are easy to copypaste into a spreadsheet.
    Args:
    ...
    assert isinstance(results, OrderedDict), results  # unordered results cannot be properly printed
    for task in results:
        print_csv_format(results[task], task)
