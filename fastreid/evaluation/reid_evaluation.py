...
class DSReidEvaluator(ReidEvaluator):
    def __init__(self, cfg, num_query, output_dir=None):
        super(DSReidEvaluator, self).__init__(cfg, num_query, output_dir)
        self.dsfeatures = []
    def process(self, inputs, outputs):
        self.pids.extend(inputs["targets"])
        self.camids.extend(inputs["camids"])
        self.dsfeatures.append([i.cpu() for i in outputs])

    def evaluate(self):
        results = OrderedDict()
        for idx, features in enumerate(zip(*self.dsfeatures)):
            self.features = features
            results["model_{}".format(idx)] = super(DSReidEvaluator, self).evaluate()
        return results