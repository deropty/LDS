...
def inference_on_dataset(model, data_loader, evaluator):
    ...
    model_was_list = True if isinstance(model, list) else False
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            if model_was_list:
                outputs = [i(inputs) for i in model]
                outputs.append(torch.cat(tuple(outputs), 1))
            else:
...
def inference_context(model):
    ...
    model_was_list = True if isinstance(model, list) else False
    if model_was_list:
        training_model = [i.training for i in model]
        [i.eval() for i in model]
        yield
        [i.train(j) for i,j in zip(model, training_model)]
    else:
