...
class LDSCheckpointer(Checkpointer):
    def __init__(
            self,
            model: nn.Module,
            save_dir: str = "",
        ...
        assert isinstance(model, list), "Model is not a list!"
        self.num_models = num_models
        if isinstance(model[0], (DistributedDataParallel, DataParallel)):
            model = [i.module for i in model]
    ...
    def save(self, name: str, **kwargs: Dict[str, str]):
        ...
        data = {}
        data["model"] = [i.state_dict() for i in self.model]
        for key, obj in self.checkpointables.items():
            if isinstance(obj, list):
                data[key] = [i.state_dict() for i in obj]
            else:
                data[key] = obj.state_dict()
        data.update(kwargs)
    ...
    def _load_model(self, checkpoint: Any):
        ...
        checkpoint_state_dict = checkpoint.pop("model")
        [self._convert_ndarray_to_tensor(i) for i in checkpoint_state_dict]

        [_strip_prefix_if_present(i, "module.") for i in checkpoint_state_dict]

        # only compare the first model
        model_state_dict = self.model[0].state_dict()
        chkpt = checkpoint_state_dict[0]
        incorrect_shapes = []
        for k in list(chkpt.keys()):
            if k in model_state_dict:
                shape_model = tuple(model_state_dict[k].shape)
                shape_checkpoint = tuple(chkpt[k].shape)
                if shape_model != shape_checkpoint:
                    incorrect_shapes.append((k, shape_checkpoint, shape_model))
                    chkpt.pop(k)

        for i in range(self.num_models):
            incompatible = self.model[i].load_state_dict(checkpoint_state_dict[i], strict=False)
    ...
    def load(self, path: str, checkpointables: Optional[List[str]] = None) -> object:
        ...
        for key in self.checkpointables if checkpointables is None else checkpointables:
            if key in checkpoint:  # pyre-ignore
                ...
                obj = self.checkpointables[key]
                if isinstance(obj, list):
                    chept = checkpoint.pop(key)
                    for i in range(self.num_models):
                        obj[i].load_state_dict(chept[i])
                else:
                    obj.load_state_dict(checkpoint.pop(key))  # pyre-ignore
    ...
    def _log_incompatible_keys(self, incompatible: _IncompatibleKeys) -> None:
        ...
        if incompatible.missing_keys:
            missing_keys = _filter_reused_missing_keys(
                self.model[0], incompatible.missing_keys
            )
            if missing_keys:
                self.logger.info(get_missing_parameters_message(missing_keys))
        if incompatible.unexpected_keys:
