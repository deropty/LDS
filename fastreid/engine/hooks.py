...
class LRScheduler(HookBase):
    ...
    def __init__(self, optimizer, scheduler):
        ...
        if isinstance(optimizer, torch.optim.Optimizer):
            ...
            param_groups = optimizer[0].param_groups
        largest_group = max(len(g["params"]) for g in param_groups)

        if largest_group == 1:
            # If all groups have one parameter,
            # then find the most common initial LR, and use it for summary
            lr_count = Counter([g["lr"] for g in param_groups])
    ...
    def after_step(self):
        optim_was_list = True if isinstance(self._optimizer, list) else False
        if optim_was_list:
            lr = self._optimizer[0].param_groups[self._best_param_group_id]["lr"]
        else:
        ...
        if next_iter < self.trainer.warmup_iters:
            if optim_was_list:
                for i in range(self.trainer.num_models):
                    self._scheduler["warmup_sched"][i].step()
            else:
    ...
    def after_epoch(self):
        ...
        if next_iter >= self.trainer.warmup_iters and next_epoch >= self.trainer.delay_epochs:
            ...
            elif isinstance(self._optimizer, list):
                for i in range(self.trainer.num_models):
                    self._scheduler["lr_sched"][i].step()
...
class LayerFreeze(HookBase):
    def __init__(self, model, freeze_layers, freeze_iters):
        ...
        self.model_was_list = isinstance(model, list)
        if self.model_was_list:
            model_was_dis = isinstance(model[0], DistributedDataParallel)
        else:
            model_was_dis = isinstance(model, DistributedDataParallel)

        if model_was_dis and self.model_was_list:
            model = [model.module for i in model]
        elif model_was_dis and not self.model_was_list:
            model = model.module
    ...
    def freeze_specific_layer(self):
        for layer in self.freeze_layers:
            if self.model_was_list and not hasattr(self.model[0], layer):
                    self._logger.info(f'{layer} is not an attribute of the model, will skip this layer')
            elif not self.model_was_list and not hasattr(self.model, layer):
                    self._logger.info(f'{layer} is not an attribute of the model, will skip this layer')
